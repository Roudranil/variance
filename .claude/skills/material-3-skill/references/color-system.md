...
MD3 defines 29+ color roles organized into groups. **Flutter:** access via `Theme.of(context).colorScheme` (e.g. `colorScheme.primary`, `colorScheme.onPrimary`).
...
The OS extracts a seed color from the user's wallpaper and generates a scheme. Android 12+ dynamic color in Flutter uses the `dynamic_color` package as shown below.

### Content-Based
A seed color is extracted from in-app content (album art, book cover, etc.) to create a contextual scheme.

### Dynamic Color in Flutter (Android 12+)

```dart
// Dynamic color (Android 12+) — requires package:dynamic_color
...
```dart
// Flutter: use the `material_color_utilities` Dart package for color harmonization.
// Blend.harmonize(customColorArgb, primaryColorArgb) — available via pub.dev.
```
...
```dart
// Flutter: pass contrast parameter to ColorScheme.fromSeed (if supported in your Flutter version)
// or use material_color_utilities Dart package directly:
// final scheme = SchemeContent(Hct.fromInt(argbFromHex('#6750A4')), false, 1.0); // high contrast
```
...
```dart
// Flutter baseline scheme — generate from seed color
...
The baseline purple seed (`#6750A4`) produces these key role values for reference:
